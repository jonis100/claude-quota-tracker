...
export class StatusBarManager {
  ...
  updateQuota(quota: QuotaInfo | null) {
    ...
    if (usagePeriod === "5-hour") {
      if (quota.fiveHour) {
        ...
        const filled = Math.round(
          (quota.fiveHour.utilization / 100) * barLength,
        );
      ...
      if (quota.sevenDay) {
        ...
        const barLength = 10;
        const filled = Math.round(
          (quota.sevenDay.utilization / 100) * barLength,
        );
        const empty = barLength - filled;
        const bar = "█".repeat(filled) + "░".repeat(empty);
    ...
    const tooltipLines = [`Claude Usage`, `─────────────────`];

    if (quota.fiveHour) {
      tooltipLines.push(
        `5-Hour Window: ${quota.fiveHour.utilization.toFixed(1)}%`,
      );
    ...
    if (quota.sevenDay) {
      tooltipLines.push(
        `7-Day Window: ${quota.sevenDay.utilization.toFixed(1)}%`,
      );
